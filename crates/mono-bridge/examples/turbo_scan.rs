...
fn main() {
    ...
    let mono = match mono_base {
        Some(b) => { println!("   mono @ 0x{b:08x}"); b }
        None => { println!("❌ mono.dll not found"); return; }
    };
    ...
    println!("   mono_get_root_domain     @ 0x{:08x}", get_root_domain.unwrap_or(0));
    println!("   mono_get_corlib          @ 0x{:08x}", get_corlib.unwrap_or(0));
    println!("   mono_class_from_name     @ 0x{:08x}", class_from_name.unwrap_or(0));
    ...
    match root_domain {
        ...
        None => { println!("❌ mono_get_root_domain failed (CreateRemoteThread blocked by ACG?)"); }
    }
    ...
    match corlib {
        ...
        None => { println!("❌ mono_get_corlib failed"); }
    }
    ...
    if let Some((ref data, _)) = domain_data {
        ...
        for off in (0..data.len().saturating_sub(12)).step_by(4) {
            ...
            if !(0x00010000..0x7FFFFFFF).contains(&ptr) { continue; }
            
            if let Ok(arr_data) = p.read_bytes(ptr, 12) {
                let len = u32::from_le_bytes([arr_data[4], arr_data[5], arr_data[6], arr_data[7]]) as usize;
                let data_ptr = u32::from_le_bytes([arr_data[8], arr_data[9], arr_data[10], arr_data[11]]) as usize;
                        ...
                        if (0x00010000..0x7FFFFFFF).contains(&assembly) {
                            ...
                            for i in 0..len.min(100) {
                                if let Ok(asm) = p.read_memory::<u32>(data_ptr + i * 4) {
                                    ...
                                    if (0x00010000..0x7FFFFFFF).contains(&asm_addr) && asm_addr != assembly {
                                        if let Ok(img) = p.read_memory::<u32>(asm_addr + 0x08) {
                                            let img_addr = img as usize;
                                            if (0x00010000..0x7FFFFFFF).contains(&img_addr) && img_addr != corlib_val {
                                                let name = read_string(&p, read_u32(&p, img_addr + 0x1C) as usize, 48);
...
fn find_export(p: &ProcessHandle, base: usize, name: &str) -> Option<usize> {
    ...
    if opt_magic != 0x10B { return None; }
    let dd = nt + 24 + 0x60;
...
fn call_remote(p: &ProcessHandle, addr: usize) -> Option<usize> {
    unsafe {
        ...
        if WaitForSingleObject(t, 5000) != 0 { CloseHandle(t); return None; }
        let mut c: u32 = 0;
        if GetExitCodeThread(t, &mut c) == 0 { CloseHandle(t); return None; }
        CloseHandle(t);
        Some(c as usize)
    }
}
...
extern "system" {
    fn CreateRemoteThread(
        h: *mut u8, _a: *mut u8, _s: usize, f: extern "system" fn() -> usize,
        _p: *mut u8, _f: u32, _tid: *mut u32,
    ) -> *mut u8;
