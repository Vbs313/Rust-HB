    ...
    for &name_off in &[0x08usize, 0x0C, 0x10] {
        ...
        while addr < 0x40000000 && found < 50 {
            ...
            while off < size && found < 50 {
                ...
                let data = match p.read_bytes(start + off, rs) {
                    Ok(d) => d,
                    _ => {
                        off += 0x10000;
                        continue;
                ...
                for i in (0..data.len().saturating_sub(0x40)).step_by(4) {
                    ...
                    if !(0x01000000..0x7FFFFFFF).contains(&np) {
                        continue;
                    }
                    ...
                    if let Ok(nb) = p.read_bytes(np as usize, 32) {
                        ...
                        if name.contains('\u{FFFD}') || name.is_empty() {
                            continue;
                        }
                        let all_ascii = name.chars().all(|c| {
                        ...
                        let img4 = u32::from_le_bytes([
                            data[i + 4],
                            data[i + 5],
                            data[i + 6],
                            data[i + 7],
                        ]);
                        let img = if img0 > 0x01000000 && img0 < 0x7FFFFFFF {
                            img0
                        } else if img4 > 0x01000000 && img4 < 0x7FFFFFFF {
                            img4
                        } else {
                            continue;
                        };
                        ...
                        let is_key = name == "SceneMgr"
                            || name == "Entity"
                            || name == "Player"
                            || name == "GameState"
                            || name.starts_with("GameStat");
