...
fn main() {
    ...
    let corlib_name = read_str(
        &p,
        p.read_memory::<u32>(corlib_img + 0x1C).unwrap_or(0) as usize,
        32,
    );
    ...
    if let Ok(data) = p.read_bytes(0x09699be0, 0x20) {
        ...
        if v8 > 0x10000 && v8 < 0x7FFFFFFF {
            let img_name = read_str(
                &p,
                p.read_memory::<u32>(v8 + 0x1C).unwrap_or(0) as usize,
                32,
            );
        ...
        if vc > 0x10000 && vc < 0x7FFFFFFF {
            let img_name = read_str(
                &p,
                p.read_memory::<u32>(vc + 0x1C).unwrap_or(0) as usize,
                32,
            );
    ...
    if let Ok(data) = p.read_bytes(0x09d25150, 0x20) {
        for off in (0..20).step_by(4) {
            ...
            if v > 0x10000 && v < 0x7FFFFFFF {
                ...
                let img_name =
                    read_str(&p, p.read_memory::<u32>(v + 0x1C).unwrap_or(0) as usize, 32);
    ...
    for &candidate in &[
        ...
        if let Ok(data) = p.read_bytes(candidate, 0x20) {
            ...
            if img > 0x10000 && img < 0x7FFFFFFF {
                let img_name = read_str(
                    &p,
                    p.read_memory::<u32>(img + 0x1C).unwrap_or(0) as usize,
                    32,
                );
                if img_name.len() > 3 {
                    println!(
                    ...
                    if let Ok(nb) = p.read_bytes(np, 48) {
                            ...
                            || name.contains("Routine")
                        {
                            ...
                            if let Ok(fb) = p.read_bytes(fp, 48) {
                                ...
                                if (fname.ends_with(".dll") || fname.ends_with(".exe"))
