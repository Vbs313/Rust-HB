...
fn main() {
    ...
    let mut results: Vec<(String, Vec<usize>)> = targets
        .iter()
            ...
            for (i, &r) in refs.iter().take(5).enumerate() {
                ...
                for name_off in [0x00usize, 0x04, 0x08, 0x0C, 0x10, 0x14, 0x18] {
                    ...
                    if let Ok(hdr) = p.read_bytes(class_start, 0x40) {
                        ...
                        if read_u32(&hdr, name_off) as usize != r {
                            continue;
                        }
                                    ...
                                    if ns.len() >= 2 && ns.len() <= 30 {
                                        ...
                                        for img_off in [0x00usize, 0x04, 0x08, 0x0C, 0x10, 0x14] {
                                            ...
                                            if img_ptr > 0x10000 && img_ptr < 0x7FFFFFFF {
                                                let img_name_ptr = p
                                                    .read_memory::<u32>(img_ptr + 0x1C)
                                                    .unwrap_or(0)
                                                    as usize;
                                                if img_name_ptr > 0x10000
                                                    && img_name_ptr < 0x7FFFFFFF
                                                {
                                                    if let Some(img_name) =
                                                        read_str_opt(&p, img_name_ptr, 48)
                                                    {
                                                        if img_name.len() > 3 {
