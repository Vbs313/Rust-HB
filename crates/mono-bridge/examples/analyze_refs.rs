...
fn main() {
    ...
    for &(ref_addr, str_addr, name) in &refs {
        ...
        for name_off in [
            0x00usize, 0x04, 0x08, 0x0C, 0x10, 0x14, 0x18, 0x1C, 0x20, 0x24, 0x28,
        ] {
            if name_off > ref_addr {
                continue;
            }
            ...
            if read_u32(&hdr, name_off) as usize != str_addr {
                continue;
            }
            ...
            for off in (0..64usize).step_by(4) {
                ...
                if val == 0 || val == 0xFFFFFFFF || val < 0x10000 || val > 0x7FFFFFFF {
                    continue;
                }
                ...
                let img_name = read_str_opt(
                    &p,
                    p.read_memory::<u32>(val + 0x1C).unwrap_or(0) as usize,
                    48,
                )
                .unwrap_or_default();
                if img_name.len() > 3 {
                ...
                if val2 > 0x10000 && val2 < 0x7FFFFFFF {
                    ...
                    let vname = read_str_opt(
                        &p,
                        p.read_memory::<u32>(val2 + 0x1C).unwrap_or(0) as usize,
                        32,
                    )
                    .unwrap_or_default();
                    if vname.len() > 3 {
