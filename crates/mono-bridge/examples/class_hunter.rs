...
fn main() {
    ...
    // 然后找游戏类
    let game_targets = ["SceneMgr", "GameState", "Entity", "GameStateManager", "GameEntity"];
...
fn find_mono_class_from_string(p: &ProcessHandle, class_name: &str) {
    ...
    let str_addr = match str_addr {
        Some(a) => a,
        None => { println!("   {class_name}: string not found"); return; }
    };
    ...
    for range_start in [0x01000000usize, 0x05000000, 0x09000000, 0x0D000000, 
                       0x11000000, 0x15000000, 0x19000000, 0x20000000,
                       0x30000000, 0x40000000] {
        scan_for_references_full(p, str_addr, range_start, range_start + 0x04000000, &mut references);
        if references.len() > 100 { break; }
    }
    ...
    for (i, &ref_addr) in references.iter().enumerate().take(5) {
        ...
        for name_off in [0x00u32, 0x04, 0x08, 0x0C, 0x10, 0x14, 0x18, 0x1C, 0x20] {
            if ref_addr < name_off as usize { continue; }
            ...
            for ns_off in [0x00u32, 0x04, 0x08, 0x0C, 0x10, 0x14, 0x18, 0x1C, 0x20] {
                ...
                if let Some(ns) = read_string_opt(p, ns_ptr, 32) {
                    ...
                    for img_off in [0x00u32, 0x04, 0x08, 0x0C, 0x10, 0x14, 0x18, 0x1C, 0x20] {
                        if img_off == name_off || img_off == ns_off || img_off as usize + 4 > hdr.len() { continue; }
                        let img_ptr = read_le32_at(&hdr, img_off as usize);
                        if img_ptr <= 0x10000 || img_ptr > 0x7FFFFFFF { continue; }
                    ...
                    for fc_off in [0x28u32, 0x2C, 0x30, 0x34, 0x38, 0x3C] {
                        if fc_off as usize + 4 > hdr.len() { continue; }
...
fn scan_for_references_full(p: &ProcessHandle, target: usize, start: usize, end: usize, refs: &mut Vec<usize>) {
    use hb_core::win32::query_memory_info;
    let mut addr = start;
    while addr < end {
        let mbi = match query_memory_info(p, addr) {
            ...
            while off < sz {
                ...
                if let Ok(buf) = p.read_bytes(s + off, rs) {
                    for j in (0..buf.len() - 4).step_by(4) {
                        let val = u32::from_le_bytes([buf[j], buf[j+1], buf[j+2], buf[j+3]]) as usize;
                        if val == target { refs.push(s + off + j); }
                    }
                }
                off += 0x40000;
            }
...
fn find_image_around(p: &ProcessHandle, target: &str, center: usize, range: usize) -> Option<usize> {
    use hb_core::win32::query_memory_info;
...
fn scan_image_class_cache(p: &ProcessHandle, img_addr: usize) {
    let data = match p.read_bytes(img_addr, 0x100) {
        Ok(d) => d,
        Err(_) => { println!("   Can't read image"); return; }
    };
...
fn try_mono_class_fast(p: &ProcessHandle, addr: usize, img_addr: usize) -> Option<(String, String)> {
    let data = p.read_bytes(addr, 0x30).ok()?;
    
    for name_off in [0x04usize, 0x08, 0x0C, 0x10, 0x14, 0x18] {
        if name_off + 8 > data.len() { continue; }
        let name_ptr = u32::from_le_bytes([data[name_off], data[name_off+1], data[name_off+2], data[name_off+3]]) as usize;
        ...
        for ns_off in [0x00usize, 0x04, 0x08, 0x0C, 0x10, 0x14, 0x18] {
            if ns_off == name_off || ns_off + 4 > data.len() { continue; }
            let ns_ptr = u32::from_le_bytes([data[ns_off], data[ns_off+1], data[ns_off+2], data[ns_off+3]]) as usize;
            ...
            if ns.len() < 2 || ns.len() > 30 || name.contains('\u{FFFD}') || ns.contains('\u{FFFD}') { continue; }
            if !ns.chars().all(|c| c.is_ascii_alphanumeric() || c == '_' || c == '.') { continue; }
            
            // Verify image pointer
            for img_off in [0x00usize, 0x04, 0x08, 0x0C, 0x10, 0x14] {
                if img_off == name_off || img_off == ns_off || img_off + 4 > data.len() { continue; }
                let img_ptr = u32::from_le_bytes([data[img_off], data[img_off+1], data[img_off+2], data[img_off+3]]) as usize;
                if img_ptr == img_addr { return Some((ns, name)); }
            }
        }
    }
    None
