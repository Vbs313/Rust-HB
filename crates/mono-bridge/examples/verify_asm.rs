                                ...
                                for no in [0x04, 0x08, 0x0C, 0x10] {
                                    ...
                                    if np > 0x10000 && np < 0x7FFFFFFF {
                                        if let Some(n) = read_str_opt(&p, np, 32) {
                                            if n.len() >= 2 && n.len() <= 30 && n.chars().all(|c| c.is_ascii_alphanumeric() || c == '_') {
                                                ...
                                                for io in [0x00, 0x04, 0x08, 0x0C] {
                                                    ...
                                                    if ip == asm_img {
                                                        ...
                                                        println!("   First class: {n} (image verified)");
                        ...
                        if key > 0x10000 && key < 0x7FFFFFFF {
                            if let Some(s) = read_str_opt(&p, key, 64) {
                                if s.contains('.') && s.len() > 5 && s.len() < 60 {
                                                    ...
                                                    if s2.contains('.') {
                                                        ...
                                                        if v > 0x10000 && v < 0x7FFFFFFF {
                                                            if let Ok(vd) = p.read_bytes(v, 0x30) {
                                                                for no in [0x04, 0x08, 0x0C, 0x10] {
                                                                    let vnp = read_u32(&vd, no) as usize;
                                                                    if vnp == k { // name pointer matches hash key
                                                                        if count < 30 {
                                                                            println!("      {s2}");
                                                                        }
                                                                        count += 1;
                                    ...
                                    let ndata = p.read_bytes(nodes, (ne * 12).min(0x40000)).unwrap_or_default();
                                    for i in 0..ne.min(5000) {
                                        let o = i * 12;
                                        if o + 12 > ndata.len() { break; }
                                        let k = read_u32(&ndata, o) as usize;
                    ...
                    if let Ok(first) = p.read_memory::<u32>(arr) {
                        ...
                        if f > 0x10000 && f < 0x7FFFFFFF {
                            if let Some(n) = read_str_opt(&p, f, 32) {
                                if n.len() >= 2 && n.chars().all(|c| c.is_ascii_alphanumeric() || c == '_' || c == '.') {
                                    println!("\n   🎯 +0x{:04X}: GPtrArray(len={}) first='{n}'", off + 0x400, len);
                                    return;
                                }
                            }
                        }
                        ...
                        if key > 0x10000 && key < 0x7FFFFFFF {
                            if let Some(s) = read_str_opt(&p, key, 64) {
                                if s.contains('.') && s.len() > 5 {
                                    println!("\n   🎯 +0x{:04X}: GHashTable(entries={}) first='{s}'", off + 0x400, ne);
                                    return;
                                }
                            }
                        }
